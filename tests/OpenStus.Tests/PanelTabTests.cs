...
public class PanelTabTests
{
    ...
    public void CtrlTOpensATabOnTheCurrentFolderAndTheStripAppears()
    {
        ...
        Assert.Equal(Theme.Classic().PanelTitle, app.Terminal.Buffer.Get(1, 1).Style);
        Assert.Equal(Theme.Classic().PanelTitleActive, app.Terminal.Buffer.Get(caption.Length + 5, 1).Style);
    }
    ...
    public void ATabRemembersItsCursorSortAndView()
    {
        ...
        Press(app, ConsoleKey.T, KeyMods.Ctrl);
        panel.SetSort(OpenStus.Files.SortMode.Size);
        ...
        Assert.Equal(OpenStus.Files.SortMode.Name, panel.SortMode);
        Assert.Equal(PanelViewMode.Medium, panel.ViewMode);
        Assert.Equal(0, panel.CursorIndex);

        Press(app, ConsoleKey.Tab, KeyMods.Ctrl, '\t');
        Assert.Equal(OpenStus.Files.SortMode.Size, panel.SortMode);
